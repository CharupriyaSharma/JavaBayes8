...
public class InferenceGraph {
  ...
  Vector<InferenceGraphNode> nodes = new Vector();
  ...
  public InferenceGraph(URL url) throws IOException, IFException {
    ...
    public void clearAllObservations(String name) {
        for (InferenceGraphNode node : nodes) {
                node.clear_observation();
        }
    }
    
    public void setObservations(String name, String value) {
        for (InferenceGraphNode node : nodes) {
            if (node.pv.get_name().equals(name)) {
